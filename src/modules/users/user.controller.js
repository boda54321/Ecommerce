...
export const signUp=asynchandler(async(req,res,next)=>{
    ...
    if (exist) {
        ...
        const token = jwt.sign({ email }, process.env.pass, { expiresIn: 60 * 5 })
    ...
    const checkSendMail = await sendEmail(email, "hello", `<a href="${link}">confirm your email</a> <br>
...
export const refConfirmEmail = asynchandler(async (req, res, next) => {
    ...
    const decoded = jwt.verify(reftoken, process.env.repass)
    if (!decoded?.email) {
...
export const signin=asynchandler(async(req,res,next)=>{
    ...
    if(!isMatch){
        ...
        const token=jwt.sign({email,id:user._id},process.env.ts)
        if (!token) {
            return next(new AppError("invalid token"))
